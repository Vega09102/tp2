...
void mcin(T* _x)
{
    while (true)
    {
        ...
        cout << "Invalid value entered" << std::endl;
        cin.clear();
        cin.ignore();
    }
}
...
void myList::edit()
{
    ...
    cout << "\tThere are " << size << " entries, select one to delete (index from 1 to " << size << ") : ";
...
void myList::findType()
{
    ...
    while (p != nullptr)
    {
        ...
        {
            cout << "Nomer: " << p->getNumber() << " Destination: " << p->getName() << "  Time " << p->getType() << endl;
        }
